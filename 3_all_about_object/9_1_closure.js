...
// call stack에서 사라지는 순서는 함수의 안쪽부터 바깥쪽
...
/**
 * innerGetNumber를 실행한 것은 getNumber가 실행된 이후
 * 이미 getNumber의 Excution Context가 끝난 이후에 runner()가 실행됨(getNumber가 call stack에서 사라진 이후)
 * -> 이것이 closure
 */
...
function Idol(name, year) {
    ...
    var _year = year;
    //this 키워드로 저장하지 않았기 때문에 객체의 프로퍼티로 access는 불가능
    // but, 생성이 완료된 다음 생성된 함수 안에서 나중에 가져올 수는 있음
